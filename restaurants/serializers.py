...
from promotions.serializers import CouponSerializer, PromotionSerializer
        ...
        if image_file:
            try:
                ...
                image_url = upload_to_cloudinary(
                    ContentFile(image_data, name=filename), filename
                )
                validated_data["image_url"] = image_url
            except Exception as e:
                raise serializers.ValidationError(
                    {"image_bytes": f"圖片處理失敗: {str(e)}"}
                )
...
class RestaurantDetailSerializer(serializers.Serializer):
    ...
    def get_promotion(self, obj):
        ...
        promotions = obj.promotions.filter(
            is_archived=False, ended_at__gte=now
        ).order_by(
            "-created_at",
        )
        return (
    ...
    def get_coupon(self, obj):
        now = timezone.now()
        coupons = obj.coupons.filter(is_archived=False, ended_at__gte=now).order_by(
            "-started_at"
        )
    ...
    def get_user_status(self, obj):
        ...
        for coupon in coupons:
            if coupon:
                result["hasClaimedCoupon"][str(coupon.uuid)] = coupon.claimed_by.filter(
